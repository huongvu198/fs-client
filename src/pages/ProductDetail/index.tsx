...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    if (firstActiveVariant) {
      setSelectorColor(firstActiveVariant.id);
      setMainImage(firstActiveVariant.images?.[0]?.url || "");
    }
  }, [productData]);
  ...
  useEffect(() => {
    ...
    if (sizes.length > 0 && !sizes.some((size) => size.id === selectedSize)) {
      const firstAvailableSize = sizes.find(
        (s) => s.inventory! > 0 && s.isActive === true
      );
              ...
              <div className={cx("product-color-selector")}>
                ...
                <div className={cx("product-color-options")}>
                  {colorOptions.map((color) => {
                    const isSelected = selectorColor === color.id;
                    const isDisabled = !color.isActive;
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          if (!isDisabled) handleColorChange(color.id);
                        }}
                        title={color.color}
                      />
                    );
              ...
              <div className={cx("product-size-selector")}>
                ...
                <RadioComponent
                  options={availableSize.map((size) => ({
                    ...
                    disabled: size.inventory === 0 || size.isActive === false,
                  }))}
                  value={selectedSize}
